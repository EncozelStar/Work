---------- [ ver1.6 : ArmonHub edition ]

local httpService = game:GetService("HttpService")

local SaveManager = {} do
	SaveManager.Folder = "ArmonHub"
	SaveManager.Ignore = {}
	SaveManager.Parser = {
		Toggle = {
        ...
        Paragraph = {
            Save = function(idx, object)
                return { type = "Paragraph", idx = idx, text = object.Value }
            end,
            Load = function(idx, data)
                if SaveManager.Options[idx] and type(data.text) == "string" then
                    SaveManager.Options[idx]:SetValue(data.text)
	...
	function SaveManager:LoadAutoloadConfig()
		...
		local configName = player.Name .. "_" .. player.UserId  -- ✅ config = PlayerName_UserId
		...
		if not success then
			return self.Library:Notify({
				...
				SubContent = "Failed to load config for player: " .. err,
				Duration = 7
			})
		end
		...
		self.Library:Notify({
			Title = "Armon Hub",
			Content = "Config loader",
			SubContent = string.format("Auto loaded config for player %q", configName),
			Duration = 7
		})
	end

    function SaveManager:AutoSaveHook(configName)
        ...
        for idx, option in pairs(self.Options) do
            ...
            task.defer(function()
                ...
                option:OnChanged(function(value)
                    self:Save(configName)
                    if typeof(originalChanged) == "function" then
                        pcall(function() originalChanged(value) end)
                    end
                end)
            end)
        end
